---
title: "DateRangePicker"
---

import ComponentPreview from '@/components/ComponentPreview';

Поле ввода для диапазона дат. Маска `дд.мм.гггг — дд.мм.гггг`, попап с двумя месяцами, двухфазный клик в календаре.

<ComponentPreview title="DateRangePicker" kind="range" />

## Базовый пример

```tsx
import { useState } from 'react'
import { DateRangePicker, type DateRange } from 'daterly'

const [range, setRange] = useState<DateRange | undefined>()

<DateRangePicker
  label="Период проживания"
  value={range}
  onChange={setRange}
/>
```

## Двухфазный выбор

Первый клик в календаре — устанавливает начало диапазона (anchor). Второй клик — фиксирует конец и закрывает попап. При движении мыши между кликами подсвечивается потенциальный диапазон.

Если кликнуть на дату _раньше_ anchor — она автоматически станет началом, а первоначальный anchor — концом.

## Расположение календаря

```tsx
<DateRangePicker calendarLayout="horizontal" />   {/* по умолчанию */}
<DateRangePicker calendarLayout="vertical" />     {/* в стек */}
```

## С выбором времени

<ComponentPreview kind="range" showTime={{ format: 'HH:mm' }} label="Период (HH:mm)" />

```tsx
<DateRangePicker
  label="Период с точностью до минуты"
  showTime={{ format: 'HH:mm' }}
/>
```

Под календарём появляются два поля выбора времени — для начала и для конца диапазона. Попап в этом режиме не закрывается автоматически — закрытие по кнопке OK.

### Варианты TimePicker

Проп `timePickerType` управляет видом выбора времени. По умолчанию — `'input'`.

**`'input'`** — два компактных числовых поля рядом с кнопкой OK (Начало / Конец). Удобно для ввода с клавиатуры.

<ComponentPreview kind="range" showTime={{ format: 'HH:mm' }} timePickerType="input" label="TimePicker: input" />

```tsx
<DateRangePicker showTime={{ format: 'HH:mm' }} timePickerType="input" />
```

**`'drum'`** — два «барабана» под календарём, листаются кликом или колесом. Подходит для тач-устройств.

<ComponentPreview kind="range" showTime={{ format: 'HH:mm' }} timePickerType="drum" label="TimePicker: drum" />

```tsx
<DateRangePicker showTime={{ format: 'HH:mm' }} timePickerType="drum" />
```

## Локаль

<ComponentPreview kind="range" locale="en" dateFormat="MM/dd/yyyy" label="Period" />

```tsx
import { enUS } from 'date-fns/locale'

<DateRangePicker locale={enUS} dateFormat="MM/dd/yyyy" label="Period" />
```

## Ограничение

```tsx
<DateRangePicker fromDate={new Date()} toDate={new Date(2027, 11, 31)} />
```

Ограничение действует и на календарь, и на ручной ввод. Недопустимые дни в календаре не кликаются. Дата, набранная или вставленная в поле вне диапазона, помечается невалидной: поле получает `aria-invalid`, а соответствующий конец диапазона уходит в `onChange` как `undefined`.

Произвольные даты блокируются через `disabledDates` — см. [DatePicker](/ru/docs/datepicker):

```tsx
<DateRangePicker disabledDates={{ dayOfWeek: [0, 6] }} />
```

Проверяются обе границы диапазона. Диапазон, перешагивающий заблокированный день, допустим — как и в календаре `react-day-picker` по умолчанию.

## Очистка

<ComponentPreview kind="range" clearable label="Период" />

```tsx
<DateRangePicker clearable label="Период" />
```

Крестик появляется, как только в поле есть значение — в том числе когда выбрана только начальная дата. Клик сбрасывает обе даты и вызывает `onChange(undefined)`.

Своя иконка — через `clearIcon`, как у [DatePicker](/ru/docs/datepicker):

```tsx
<DateRangePicker clearable clearIcon={<MyCloseIcon />} />
```

## Все пропсы

Те же, что и у [DatePicker](/ru/docs/datepicker), плюс:

| Prop             | Тип                                       | По умолчанию   | Описание                  |
| ---------------- | ----------------------------------------- | -------------- | ------------------------- |
| `value`          | `DateRange`                               | —              | `{ from, to }`            |
| `defaultValue`   | `DateRange`                               | —              | Начальный диапазон        |
| `onChange`       | `(range: DateRange \| undefined) => void` | —              | Колбэк                    |
| `calendarLayout` | `'horizontal' \| 'vertical'`              | `'horizontal'` | Расположение двух месяцев |

`DateRange` импортируется из основного entry-point:

```tsx
import type { DateRange } from 'daterly';
```
